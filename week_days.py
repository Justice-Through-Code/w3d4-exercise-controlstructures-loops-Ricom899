...
def favorite_activities():
    ...
    for x in days:
        user_answer = input(f'What is your favorite thing to do on {x}s?')
        favorite_activities.append(user_answer)
    ...
    for x in range(len(days)):
        print(f'On {days[x]}s, your favorite activity is to {favorite_activities[x]}.')
...
def temp_by_day():
    ...
    for x in days:
        user_tem = int(input('what is the temperature?'))
        if user_tem < 50:
            print('Brr, put on a jacket!')
        elif user_tem >= 50 and user_tem <= 65:
